...
public class ProductManager {
    ...
    // Método para listar produtos abaixo do limite mínimo
    public static void listProductsBelowLimit() {
        String sql = "SELECT * FROM produtos WHERE quantidade < limite_minimo";

        try (Connection conn = DatabaseConnection.connect();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(sql)) {

            // Iterar sobre o ResultSet para mostrar os produtos abaixo do limite
            while (rs.next()) {
                System.out.println(
                        "ID: " + rs.getInt("id") +
                        ", Nome: " + rs.getString("nome") +
            ...
            System.out.println("Erro ao consultar os produtos abaixo do limite: " + e.getMessage());
        }
    }

    // Método para obter descrições dos produtos para exibição
    public static List<String> getProductDescriptions() {
        List<String> descriptions = new ArrayList<>();
        String sql = "SELECT id, nome, quantidade, limite_minimo, descricao FROM produtos";
        ...
        try (Connection conn = DatabaseConnection.connect();
            ...
            while (rs.next()) {
                String description = "ID: " + rs.getInt("id") + ", Nome: " + rs.getString("nome") + 
                                     ", Quantidade: " + rs.getInt("quantidade") + 
                                     ", Limite Mínimo: " + rs.getInt("limite_minimo") + 
                                     ", Descrição: " + rs.getString("descricao");
            ...
            System.out.println("Erro ao obter descrições dos produtos: " + e.getMessage());
        }
        return descriptions;
    }
}
